...
function Cart() {
    ...
    const onClearCart = () => {
        if (window.confirm('Вы действительно хотите отчистить корзину?')) dispatch(clearCart());
    }
    ...
    const onClickOrder = () => {
        alert(`Заказ можно увидеть в консольке)`);
        console.log(items);
    }
    ...
    return <div className="container container--cart">
        {
            totalCount ? <div className="cart">
                ...
                <div className="content__items">
                    {
                        addedPizzas.map((obj, key) => <CartItem key={`${obj}_${key}`} id={obj.id} name={obj.name} type={obj.type}
                                                                size={obj.size} totalPrice={items[obj.id].totalPrice}
                                                                totalCount={items[obj.id].items.length} onRemove={onRemoveItem} onMinus={onMinusItem} onPlus={onPlusItem}/>)
                    }
                </div>
